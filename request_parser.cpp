...
bool request_parser::is_body(request& req)
{
  std::vector<header>::iterator it = req.headers.begin();
  for ( ; it != req.headers.end(); ++it )
  {
    if ( (*it).name == "Content-Length" )
    {
      req.body_len  = boost::lexical_cast<std::size_t>((*it).value.c_str());
      return true;
    }
  }
  return false;
